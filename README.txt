...
The work breakdown was evenly split. Austin implemented volsort.h and list.cpp, as well as qsort.cpp and stl.cpp with minor edits from Jordan. Jordan completed merge.cpp, wrote number_gen.cpp to create text files with random numbers, and wrote runtime.py to save benchmarking data. Mathew implemented quick.cpp with final bugtesting by Jordan and wrote Table.cpp to create the datatable containing benchmarking data. Austin also completed the questions with input from the others and everyone communicated about the writeup. We also commented the code we wrote. We were able to have clear communication the whole time and overall worked very well together.
